...
bool CAlarmVideoAnalyse::WinDataToStruct(int nChannelNum)
{
	...
	CAlarmInputDlg::GetButtonsCheck(m_pBtnsRecord, eventHander.dwRecord, nChanneCount);		// 录象掩码
	eventHander.iRecordLatch = GetEditValue(IDC_EDIT_RECORD_IG_TIME).Toint();		// 录像延时：10～300 sec 
	CAlarmInputDlg::GetButtonsCheck(m_pBtnsTour, eventHander.dwTour, nChanneCount);			// 轮巡掩码
