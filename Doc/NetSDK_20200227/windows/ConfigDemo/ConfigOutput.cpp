...
void CConfigOutput::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_Control(pDX, IDC_COMBO_VIDEORESOLUTION, m_VideoResolution);
}
...
BEGIN_MESSAGE_MAP(CConfigOutput, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_CBN_SELCHANGE(IDC_COMBO_VIDEORESOLUTION, &CConfigOutput::OnCbnSelchangeComboVideoresolution)
END_MESSAGE_MAP()
...
void CConfigOutput::InitDlgInfo(OUTPUTCONFIGALL* pOutputCfgAll,SDK_VGAresolution* VGAresolution,int nbyChanNum)
{
	 ...
	 if(NULL != VGAresolution)
	 {
		memcpy(&m_OutVGAresolution, VGAresolution, sizeof(SDK_VGAresolution));
	 }
	 else
	 {
		memset(&m_OutVGAresolution, 0, sizeof(SDK_VGAresolution));
	 }
     InitGUISetCfgInfo();
	 InitVidWdgCfgInfo();
	 InitVGAResAbiInfo();
...
void CConfigOutput::InitVGAResAbiInfo()
{
	if ( !((CClientDemo5Dlg*)AfxGetMainWnd())->m_bOutputCfg[3])
	{
		GetDlgItem(IDC_STATIC_RESOLUTION)->EnableWindow(FALSE);
		m_VideoResolution.EnableWindow(FALSE);
	}
	else
	{
		GetDlgItem(IDC_STATIC_RESOLUTION)->EnableWindow(TRUE);
		m_VideoResolution.EnableWindow(TRUE);
		
		m_VideoResolution.ResetContent();
		for(int j=0; j<m_outputCfgAll.VGAResolutionAbility.vVGANum; ++j)
		{
			m_VideoResolution.AddString(m_outputCfgAll.VGAResolutionAbility.vVGAList[j]);
		}
		char curVGA[VGA_NAME_LENGTH];
		sprintf(curVGA,"%d%s%d",m_OutVGAresolution.nWidth,"*",m_OutVGAresolution.nHeight);
		m_VideoResolution.SelectString(-1,curVGA);
	}
}
...
void CConfigOutput::OnBtnApply() 
{
	...
	{
		CString CurVGA;
		m_VideoResolution.GetLBText(m_VideoResolution.GetCurSel(), CurVGA);
		int i = CurVGA.Find("*");
		m_OutVGAresolution.nHeight = atoi(CurVGA.Mid(i+1));
		m_OutVGAresolution.nWidth  = atoi(CurVGA.Mid(0,i));

		int nRet = H264_DVR_SetDevConfig(lLoginID,E_SDK_CFG_VIDEOOUT, -1, 
						(char*)&m_OutVGAresolution, sizeof(SDK_VGAresolution), 10000);
		if(nRet > 0)
		{
...
void CConfigOutput::OnCbnSelchangeComboVideoresolution()
{
	// TODO: 在此添加控件通知处理程序代码
	if ( ((CClientDemo5Dlg*)AfxGetMainWnd())->m_bOutputCfg[3])
	{
		int nIndex = m_VideoResolution.GetCurSel();
		int nWidht = m_VideoResolution.GetItemData(nIndex);
	}
}