...
#include "UpgradeDemoDlg.h"		//add 头文件
#include "ManageTool.h"		//add 管理工具头文件
#include "ContrlRecord.h"	//add 摄像控制头文件
#include "ConfigPtz.h"
...
// Dialog Data
	...
	enum { IDD = IDD_CONFIGDEMO_DIALOG };
...
protected:
	...
	afx_msg void OnTimer(ONTIMER_UINT nIDEvent);
...
public:
	...
	CConfigPtz m_Ptz;
	CSysSettingCamera m_SysCamera; //摄像机参数
	//by zhouhao
    ...
    CConfigVehicle m_vehicleDlg; //车载功能
	CConfigDigitalChannel* m_cfgDigitalChannel;
    CToolManagerHardDisk m_toolManagerHardDiskDlg;
	
	CSnapStorageDlg  m_SnapStorageDlg;
	
	CManageTool m_ManageToolDlg;	//管理工具
	CUpgradeDemoDlg m_UpgradeDemoDlg;
	CContrlRecord m_ContrlRecord;	//摄像控制

	int m_RecordChannel[64];	//支持辅码流设置的通道

	void GetUpgradeDemoDlg();	//获取系统升级
	void GetManageToolDlg();	//管理工具
	void GetRecordContrlInfo();
	...
	void SetAlarmException(SDK_VIDEOLOSSCONFIG *pStorageNE,SDK_StorageFailConfig *pStorageFL,SDK_VIDEOLOSSCONFIG *pNetipCfg,SDK_VIDEOLOSSCONFIG *pNetaBort,SDK_StorageLowSpaceConfig *pStorageLSCfg);
...
public:
	void GetDate(void);
	void Login();
	afx_msg void OnBnClickedBtnSearshDevice();
};
