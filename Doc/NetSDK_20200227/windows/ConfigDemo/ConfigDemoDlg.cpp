...
// ConfigDemoDlg.cpp : implementation file
//
...
, m_addrStr(_T(""))
{
	...
	memset(&m_sysFunc,0,sizeof(m_sysFunc));
}
...
ON_BN_CLICKED(IDC_BTN_SEARSH_DEVICE, &CClientDemo5Dlg::OnBnClickedBtnSearshDevice)
END_MESSAGE_MAP()
...
BOOL CClientDemo5Dlg::OnInitDialog()
{
	...
	m_nbyChanNum=0;
	m_niDigChannel = 0;
	memset(m_RecordChannel, 0, sizeof(m_RecordChannel));
...
void CClientDemo5Dlg::Login()
{
	...
	if(bValid)
	{
		...
		if(lRet>0)
		{
			...
			H264_DVR_GetDevConfig(m_LoginID, E_SDK_CONFIG_ABILITY_SYSFUNC, -1, (char*)&m_sysFunc,sizeof(m_sysFunc),&lRetLen,5000);
			//by zhouhao
			...
			//判断是否设置自我维护功能，如果设置，判断自动重启的时间，以及是否自动删除旧文件
			//GetDate();
			SDK_AbilityMask nMask;
			CString str;
			int i = 0;
			nSelect = H264_DVR_GetDevConfig(m_LoginID, E_SDK_ABILITY_SUPPORT_EXTSTREAM, -1,
												(char*)&nMask, sizeof(SDK_AbilityMask),&lRetLen);
			for(int cnt = 0; cnt < 8; ++cnt)
			{
				if(nMask.AbilityPram >> cnt & 0x01)
				{
					m_RecordChannel[cnt] = 1;
				}
			}
			//Device property 
//		    GetDeviceConfigInfo();	
...
//计算天数
void CClientDemo5Dlg::GetDate(void)
{
	SDK_AutoMaintainConfig AutoConfig;
	unsigned long lRetLen;
	int nRet = H264_DVR_GetDevConfig(m_LoginID, E_SDK_CONFIG_AUTO, -1,
		(char*)&AutoConfig, sizeof(SDK_AutoMaintainConfig), &lRetLen, 5000);
	if(nRet > 0)
	{
		//获取成功，判断天，时间
		if(AutoConfig.iAutoRebootDay > 1 || AutoConfig.iAutoRebootDay <= 8
			&& AutoConfig.iAutoRebootHour >= 0 || AutoConfig.iAutoRebootHour >= 23)
		{
			//设备需在什么时候重启
			CString strHour;
			CString strDate;
			...
			int WeekRet = AutoConfig.iAutoRebootDay - atoi(tm.Format("%w")) - 2;	//星期
			if(WeekRet < 0)
			{
				WeekRet = 9 - atoi(tm.Format("%w")) + AutoConfig.iAutoRebootDay;
			}
			int HourRet = 23 - atoi(tm.Format("%H")) + AutoConfig.iAutoRebootHour;		//时间
			if(HourRet < 0)
			{
				HourRet += 24;
				WeekRet--;
			...
			strDate.Format("%d", WeekRet);
			strHour.Format("%d", HourRet);
			strMin.Format("%d", 60 - atoi(tm.Format("%M")));
			strDate = strDate + _CS("Vesion.Day") + strHour + _CS("Playback.Hours") + strMin;
			MessageBox(_CS("Error.DeviceIn") + strDate + _CS("Error.MinRestart"));
			m_AutoRestTime = (WeekRet*24 + HourRet)*60 + atoi(tm.Format("%M"));	//获得自动重启的总时间
			SetTimer(1, 60000, NULL);
		}
		//是否需要自动删除旧文件
		if(AutoConfig.iAutoDeleteFilesDays)
		{
			//删除
			MessageBox("需要删除旧文件");
		}
...
void CClientDemo5Dlg::OnBtnLogout() 
{
	...
	if(bRet)
	{
		...
		m_UpgradeDemoDlg.CleanAll();//清除升级文件信息
		m_ContrlRecord.CleanAll();	
	}
}
...
void CClientDemo5Dlg::InitTabControl()
{
	...
	//增加摄像设置
	m_ContrlRecord.Create(IDD_CONFIG_CONTRL, &m_ctlMainTab);
	m_ContrlRecord.MoveWindow(childRect);
...
//	m_DigChannelDlg.MoveWindow(childRect);
	...
	//增加管理工具项
	m_ManageToolDlg.Create(IDD_DLG_TOOL_MANAGE, &m_ctlMainTab);
	m_ManageToolDlg.MoveWindow(childRect);
	...
	//增加系统升级对话框
	m_UpgradeDemoDlg.Create(IDD_DLG_UpGrade,&m_ctlMainTab);
	m_UpgradeDemoDlg.MoveWindow(childRect);
	...
	m_ctlMainTab.InsertItem(4, _CS("Title.RecordMode"));
	
	m_ctlMainTab.InsertItem(5,_CS("Title.SnapShot"));

	m_ctlMainTab.InsertItem(6, _CS("Title.NetCommonConfig"));
	m_ctlMainTab.InsertItem(7, _CS("Title.AlarmConfig"));
	m_ctlMainTab.InsertItem(8,_CS("Title.PtzConfig"));
	m_ctlMainTab.InsertItem(9, _CS("Title.CameraPara"));
	m_ctlMainTab.InsertItem(10, _CS("Title.UserManager"));
	m_ctlMainTab.InsertItem(11,_CS("Title.ChannelName"));
	m_ctlMainTab.InsertItem(12,_CS("Title.CarFunc"));
    m_ctlMainTab.InsertItem(13,_CS("Title.DigitalChannel"));
	m_ctlMainTab.InsertItem(14,_CS("Title.DisplayConfig"));
	m_ctlMainTab.InsertItem(15,_CS("Title.LogInfo"));
	//by zhouhao
	m_ctlMainTab.InsertItem(16,_CS("Title.NetService"));
	m_ctlMainTab.InsertItem(17,_CS("Title.HDDManager"));
	m_ctlMainTab.InsertItem(18,_CS("Title.VideoAnalyse"));
	m_ctlMainTab.InsertItem(19,_CS("Title.Tools"));				//增加管理工具
	m_ctlMainTab.InsertItem(20,_CS("Title.Version"));

	m_ctlMainTab.InsertItem(21,_CS("Title.SystemUpgrade"));		//系统升级
...
void CClientDemo5Dlg::DoTab(int nTab)
{
	...
	BOOL bTab[22];
	bTab[0]=bTab[1]=bTab[2]=bTab[3]=bTab[4]=bTab[5]=bTab[6]=bTab[7]=bTab[8]=bTab[9]=bTab[10]=bTab[11]=bTab[12]=bTab[13]=bTab[14]=bTab[15]=bTab[16]=bTab[17]=bTab[18]=bTab[19]=bTab[20]=bTab[21]=FALSE;
	bTab[nTab]=TRUE;
	if ( m_LoginID > 0 )
	{
		if ( nTab == 0)
	...
	SetDlgState(&m_ContrlRecord, bTab[4]);
	SetDlgState(&m_SnapStorageDlg,bTab[5]);
	SetDlgState(&m_networkDlg,bTab[6]);
	SetDlgState(&m_alarmDlg,bTab[7]);
	SetDlgState(&m_Ptz,bTab[8]);
	SetDlgState(&m_SysCamera, bTab[9]);
	SetDlgState(m_userManager, bTab[10]);
	SetDlgState(&m_channelNS,bTab[11]);
	SetDlgState(&m_vehicleDlg,bTab[12]);
	SetDlgState(m_cfgDigitalChannel,bTab[13]);
	SetDlgState(&m_outputDlg,bTab[14]);
    SetDlgState(&m_logInfoDlg,bTab[15]);
	SetDlgState(&m_netServerDlg,bTab[16]);
    SetDlgState(&m_toolManagerHardDiskDlg,bTab[17]);
	SetDlgState(m_alarmVideoAnalyse,bTab[18]);
	SetDlgState(&m_ManageToolDlg, bTab[19]);
	SetDlgState(&m_InfoVersion,bTab[20]);
	SetDlgState(&m_UpgradeDemoDlg,bTab[21]);	//设置系统升级对话框
	if(NULL != m_dlgDeviceType)
	{
...
BOOL g_bSignal = TRUE;
void CClientDemo5Dlg::OnTimer(ONTIMER_UINT nIDEvent) 
{
	...
	if(m_AutoRestTime--)
	{
		H264_DVR_ControlDVR(m_LoginID, 0, 10000);		//重启
		OnBtnLogout();
	}
...
void CClientDemo5Dlg::SetDeviceConfigInfo(DEV_ATTRIBUTE *pDevAttribute)
{
	...
	if (0 != m_LoginID && pDevAttribute != NULL)
	{
		...
		if ( bSuccess && dwRetLen == sizeof(SDK_CONFIG_NORMAL))
		{
			...
			memcpy(&NormalConfig.sysTime, &pDevAttribute->NormalConfig.sysTime,sizeof(SDK_SYSTEM_TIME));
			
			BOOL  bSuccess = H264_DVR_SetDevConfig(m_LoginID,E_SDK_CONFIG_SYS_TIME,-1,(char *)&NormalConfig.sysTime,sizeof(SDK_SYSTEM_TIME),nWaitTime);
			bSuccess = H264_DVR_SetDevConfig(m_LoginID,E_SDK_CONFIG_SYSNORMAL,0,(char *)&NormalConfig,sizeof(SDK_CONFIG_NORMAL),nWaitTime);
...
void CClientDemo5Dlg::SetNetWorkConfigInfo(SDK_CONFIG_NET_COMMON *pNetCommon, SDK_NetDNSConfig *pNetDNS, SDK_NetDHCPConfigAll *pNetDHCP)
{
	if (0 != m_LoginID && pNetCommon != NULL && pNetDNS != NULL && pNetDHCP != NULL)
	{
		...
		if (bSuccess && dwRetLen == sizeof (SDK_CONFIG_NET_COMMON))
		{
			...
			strcpy(NetWorkCfg.HostName, pNetCommon->HostName);
...
void CClientDemo5Dlg::GetCameraPara()
{
	...
	{
		m_typeIPC = false;
		MessageBox(_CS("Desktop.UnsupportCamera"));
		return;
	}
	if ( 0 != m_LoginID)
	{
...
void  CClientDemo5Dlg::SetAlarmException(SDK_VIDEOLOSSCONFIG *pStorageNE,SDK_StorageFailConfig *pStorageFL,SDK_VIDEOLOSSCONFIG *pNetipCfg,SDK_VIDEOLOSSCONFIG *pNetaBort,SDK_StorageLowSpaceConfig *pStorageLSCfg)
{
...
// 		BOOL bReboot = FALSE;
		...
		AlarmExp_SC = H264_DVR_SetDevConfig(m_LoginID,E_SDK_CONFIG_STORAGEFAILURE,-1,
			(char *)pStorageFL,sizeof(SDK_StorageFailConfig),nWaitTime);
...
void CClientDemo5Dlg::GetAlarmException()
{
	if(0!=m_LoginID)
	{
		...
		if (bSuccess&&dwRetLen==sizeof(SDK_VIDEOLOSSCONFIG))
		{
			...
			if (bSuccess&&dwRetLen==sizeof(SDK_SystemFunction))
			{
				SDK_StorageFailConfig *StorageFL=new SDK_StorageFailConfig;
				bSuccess = H264_DVR_GetDevConfig(m_LoginID,E_SDK_CONFIG_STORAGEFAILURE,-1,
				(char *)StorageFL,sizeof(SDK_StorageFailConfig),&dwRetLen,nWaitTime);
				if (bSuccess&&dwRetLen==sizeof(SDK_StorageFailConfig))
				{
...
void CClientDemo5Dlg::setChannelNS(SDK_ChannelNameConfigAll *pChannelName)
{
	if (0 != m_LoginID)
	{
		...
		{
			SDK_TitleDot* titleDot = new SDK_TitleDot;
			memset(titleDot,0,sizeof(SDK_TitleDot));
			titleDot->height = m_nHeigth;
			titleDot->width = m_nWidth;
			memcpy(&titleDot->pDotBuf,(char*)m_pBuf,m_nWidth*m_nHeigth/8);
			H264_DVR_SetDevConfig(m_LoginID,E_SDK_CONFIG_CHANNELTILE_DOT,0,
			(char *)titleDot, sizeof(SDK_TitleDot),nWaitTime);
		}
...
{
	if (!m_sysFunc.vMobileCarFunction[SDK_MOBILEDVR_STATUS_EXCHANGE] && !m_sysFunc.vMobileCarFunction[SDK_MOBILEDVR_GPS_TIMING])
	{
		MessageBox(_CS("Desktop.UnsupportCar"));
		return;
	}
	if (0!=m_LoginID)
	{
		...
		if (bSuccess&&dwRetLen==sizeof(SDK_CarStatusExchangeAll))
		{
			...
			{
				m_vehicleDlg.InitDlgInfo(&pCarStaExg,&pCarDelayTimeCfg,&m_sysFunc,m_nAlarmInNum);
			}
...
void CClientDemo5Dlg::GetNetDecorderCfg()
{
	if (m_niDigChannel == 0)
	{
		MessageBox(_CS("Desktop.UnsupportDigital"));
		return;
	}
...
void CClientDemo5Dlg::GetOutputConfigInfo()
{
	if (0 != m_LoginID)
	{
	  ...
	  SDK_VGAresolution VGAresolution;
	  VGAresolution.nHeight = 0;
	  VGAresolution.nWidth  = 0;
	  int numChannel = 0;	//针对IPC，与DVR设备每个通道的不同设置大小
	  if(!m_typeIPC)
	  {
		  numChannel = m_niDigChannel;
	  }
	  else
	  {
		  numChannel = m_nChannelNum;
	  }
	 // 针对个别设备配置不同		
	  BOOL bSuccess = FALSE;
	  bSuccess = H264_DVR_GetDevConfig(m_LoginID, E_SDK_CONFIG_CHANNEL_NAME,-1,
	  ...
	  if(TRUE == bSuccess && sizeof(SDK_ChannelNameConfigAll) == dwRetLen && 0 < numChannel)
	  {
		  m_bOutputCfg[0] = TRUE;
	  }
	  else
	  ...
	  if(!m_typeIPC)	//不是IPC则添加分辨率
	  {
		  int nRet = H264_DVR_GetDevConfig(m_LoginID, E_SDK_CFG_ABILITY_VGARESOLUTION, -1, 
			  (char*)&OutputCfgAll->VGAResolutionAbility, sizeof(SDK_VGAResolutionAbility), &dwRetLen, nWaitTime);
		  if(TRUE == nRet && sizeof(SDK_VGAResolutionAbility) == dwRetLen)
		  {
			  nRet = H264_DVR_GetDevConfig(m_LoginID,E_SDK_CFG_VIDEOOUT, -1, 
					(char*)&VGAresolution, sizeof(SDK_VGAresolution), &dwRetLen, nWaitTime);
			  if(nRet == 1 || nRet == 2)
			  {
				  m_bOutputCfg[3] = TRUE;
			  }
	  ...
	  for (int i = 0;i < 4;++ i)
	  {
		  if (m_bOutputCfg[i])
		  {
			  m_outputDlg.InitDlgInfo(OutputCfgAll,&VGAresolution,m_nbyChanNum + m_niDigChannel);
		  }
	  }
	  delete OutputCfgAll;
	}
	  ...
	  if (bSuccess)
	  {
			...
			if ( bReboot == TRUE )
			{
				if ( MessageBox(_CS("ConfigNet.NeedReboot"), _CS("OneClick.Prompt"), MB_YESNO | MB_ICONQUESTION ) == IDYES)
				{
					...
					{
						MessageBox(_CS("Error.RebootFail"), _CS("OneClick.Prompt"));
					}
				}
			}
			if ( bSuccess > 0)
...
{
	if (!m_sysFunc.vAlarmFunction[SDK_ALARM_FUNCTION_TYPE_VIDEOANALYSE])
	{
		m_alarmVideoAnalyse->m_bAnalyse = false;
		MessageBox(_CS("Desktop.UnsupportAnalyse"));
		return;
	}
...
void CClientDemo5Dlg::GetUpgradeDemoDlg()
{
	unsigned long lRetLen;
	CWnd * pWnd = m_UpgradeDemoDlg.GetDlgItem(IDC_STATIC_UpgardeFile);
	//SDK_NetWifiDeviceAll m_NetWifiDeviceAll;
	//bSuccess = H264_DVR_GetDevConfig(loginID, E_SDK_CONFIG_NET_WIFI_AP_LIST, -1,
	//	(char*)&m_NetWifiDeviceAll, sizeof(SDK_NetWifiDeviceAll), &lRetLen, 5000);
	//if(bSuccess < 0)
	//{
	//	return;
	//}
	//获取升级信息/参数/文件名
	//SDK_UpgradeInfo m_UpgradeInfo;
	//bSuccess = H264_DVR_GetDevConfig(loginID, E_SDK_CONFIG_UPGRADEINFO, -1, (char*)&m_UpgradeInfo, sizeof(SDK_UpgradeInfo), &lRetLen, 5000);
	
	if(m_sysFunc.vOtherFunction[SDK_OTHER_SUPPORT_CLOUD_UPGRADE] == TRUE)
	{
		//说明支持云升级
		//获取升级文件列表
		int ret = H264_DVR_GetDevConfig(loginID, E_SDK_CFG_UPGRADE_VERSION_LIST, -1,
					(char*)&m_UpgradeDemoDlg.m_UpgradeList, sizeof(m_UpgradeDemoDlg.m_UpgradeList), &lRetLen, 1000);
		if(ret > 0)
		{
			//m_UpgradeDemoDlg.m_strUpgradeFile.Format("%s", m_UpgradeDemoDlg.m_UpgradeList);
			pWnd->SetWindowText(m_UpgradeDemoDlg.m_UpgradeList.versions->name);
		}
		else
		{
			/*m_UpgradeDemoDlg.m_strUpgradeFile*/
			pWnd->SetWindowText(_CS("Error.NewVersion"));
		}
	}
	else
	{
		pWnd->SetWindowText(_CS("Error.NotSupportCloudUpgrade"));
	}
}
void CClientDemo5Dlg::GetManageToolDlg()
{
	if ( 0 != m_LoginID)
	{
		m_ManageToolDlg.Control();
	}
}
void CClientDemo5Dlg::GetRecordContrlInfo()
{
	for(int i=0; i<sizeof(m_RecordChannel); ++i)
	{
		if(!m_RecordChannel[i])
		{
...
void CClientDemo5Dlg::OnBnClickedBtnSearshDevice()
{
	// TODO: 在此添加控件通知处理程序代码
	CDeviceSearch* dlg = new CDeviceSearch;
	if(IDOK == dlg->DoModal())
	{
		m_addrStr = dlg->m_strIP;
		m_strPwd = dlg->m_strPsw;
		m_strUserName = dlg->m_strUserName;
		SetDlgItemText(IDC_EDIT_ADDRESS, dlg->m_strIP);
		SetDlgItemText(IDC_EDIT_DVRPWD, dlg->m_strPsw);
		SetDlgItemText(IDC_EDIT_USERNAME, dlg->m_strUserName);
		Login();
	}
}
