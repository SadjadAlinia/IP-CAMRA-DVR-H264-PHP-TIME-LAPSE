...
BOOL CAlarmVLostDlg::SaveVlossInfo()
{
	...
	if (nIndex == m_nChannelNum)
	{
		...
		{
			SDK_EventHandler &eventHander = m_VLossCfgAll.vGenericEventConfig[i].hEvent;
			...
			CAlarmInputDlg::GetButtonsCheck(m_pBtnsAlarmOut, eventHander.dwAlarmOut, m_nAlarmOutNum);
			CAlarmInputDlg::GetButtonsCheck(m_pBtnsRecord, eventHander.dwRecord, m_nChannelNum);		// 录象掩码
			CAlarmInputDlg::GetButtonsCheck(m_pBtnsTour, eventHander.dwTour, m_nChannelNum);
			CAlarmInputDlg::GetButtonsCheck(m_pBtnsSnap, eventHander.dwSnapShot, m_nChannelNum);
			...
			for (int j = 0; j < m_nChannelNum; ++j)
			{
				...
				if(eventHander.dwSnapShot[j])
					eventHander.bSnapEn = true;			// 抓图使能	
			}
		}
	}else
	{
		SDK_EventHandler &eventHander = m_VLossCfgAll.vGenericEventConfig[m_nLastChannel].hEvent;
