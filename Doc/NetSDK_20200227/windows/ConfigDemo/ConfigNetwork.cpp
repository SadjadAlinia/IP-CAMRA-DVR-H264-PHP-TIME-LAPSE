...
void CConfigNetwork::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_Text(pDX, IDC_EDIT_Dev, m_strDevName);
}
...
void CConfigNetwork::InitDlgInfo(SDK_CONFIG_NET_COMMON *pNetCommon, SDK_NetDNSConfig *pNetDNS, SDK_NetDHCPConfigAll *pNetDHCP,SDK_NetOrderConfig *pNetOrder,SDK_NetOrderFunction *pNetOrderFunc)
{
	...
	nIndex = m_ctlTransferPolicy.AddString(_CS("ConfigNet.TransMission"));
	m_ctlTransferPolicy.SetItemData(nIndex,3);
	...
	m_strDevName = pNetCommon->HostName;
	SetDlgItemText(IDC_EDIT_Dev,(LPCSTR)(LPSTR)m_strDevName.GetBuffer(0));
	//mac
...
void CConfigNetwork::SaveNetCfgInfo()
{
	...
	strcpy(m_pNetCommon.HostName,m_strDevName);
}
