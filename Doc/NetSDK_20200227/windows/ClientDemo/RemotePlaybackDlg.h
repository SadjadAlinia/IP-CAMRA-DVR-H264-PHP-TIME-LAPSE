...
protected:
	...
	long m_nPlaybackDecHandle[64];	//handle of file play,decode
...
private:
	...
	long m_nPlayHandle[64];		//handle of file play,original data
...
public:
	...
	int m_nGetPage;
	int m_nCurRecNum;
	int m_TotalChannel;					//获取当前设备的通道个数
	unsigned __int64 m_SelectChannel;	//获取选择通道后的通道号
	H264_DVR_FINDINFO m_nSearchInfo;
	CRecordPeriodDlg m_TimeReplayerView;
	CPlayWindowManager *m_playWindowManager;
	...
	afx_msg void OnBnClickedBtnSelectchannle();
	SDK_SearchByTimeResult m_SearchByTimeInfo;
	struct DEC
	{
		long pthis;
		long decHandle;
