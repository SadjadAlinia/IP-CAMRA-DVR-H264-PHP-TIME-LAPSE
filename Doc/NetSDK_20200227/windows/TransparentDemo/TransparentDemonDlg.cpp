...
//  	buf[1]=0x10;	
		...
		if (m_hex)
		{
			...
			if(m_b485)
			{
				nRet = H264_DVR_SerialWrite(m_loginID,RS485,data,len);
			}
			else
			{
				nRet = H264_DVR_SerialWrite(m_loginID,RS232,data,len);
			}
		}else
		{
			if(m_b485)
			{
				nRet = H264_DVR_SerialWrite(m_loginID,RS485,m_strWrite.GetBuffer(0),m_strWrite.GetLength()+1);
			}
			else
			{
				nRet = H264_DVR_SerialWrite(m_loginID,RS232,m_strWrite.GetBuffer(0),m_strWrite.GetLength()+1);
			}
		}
		if (nRet > 0)
		{
...
void CTransparentDemonDlg::OnTimer(ONTIMER_UINT nIDEvent) 
{
